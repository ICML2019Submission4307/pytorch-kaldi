...
def write_out_fol(config, i, drop, block_size, out_folder, k):
    # out_folder = 'exp/TEDLIUM_LSTM_fmllr_' + str(i) + 'c_uni_2l_hcgs_' + str(drop[0]) + '_' + str(
    ...
    config['architecture1']['lstm_lay'] = str(i)+','+str(i)+','+str(i)
    config['architecture1']['param_quant'] = str(k)+','+str(k)+','+str(k)
    config['architecture2']['param_quant'] = str(k)
    config_file = 'cfg/TEDLIUM/run_sweep.cfg'
        ...
        for j in total_drop:
            if j == 50:
                ...
                while block_size[0] != 2:
                    out_folder = 'exp/TEDLIUM_LSTM_fmllr_' + str(i) + 'c_uni_2l_hcgs_' + str(drop[0]) + 'd' + str(
